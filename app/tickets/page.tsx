...
export default function RSVPPage() {
  ...
  // Fetch live bed availability
  useEffect(() => {
    fetch('/api/beds')
      .then(res => res.json())
      .then(data => setBeds(data))
      .catch(() => {
        // Fallback if API not configured yet
        setBeds({
          ...
          <div className="bg-[#D4726A] rounded-3xl p-8 border-4 border-[#2D4A3E] min-h-[400px] flex flex-col">
              ...
              <div className="flex-1">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { value: 'day-only', label: 'Just for the day', disabledWhen: [] as string[] },
                      { value: 'one-night', label: 'One night', disabledWhen: [] as string[] },
                      { value: 'two-nights', label: 'Two nights (Fri\u2013Sun)', disabledWhen: ['saturday'] },
                      { value: 'unsure-stay', label: 'Not sure yet', disabledWhen: [] as string[] },
                    ].map((option) => {
            ...
            {step === 3 && (
              <div className="flex-1">
                    ...
                    {[
                      {
                        ...
                        availability: `${castleBedsRemaining} of ${totalCastleBeds} beds available`,
                      },
                      {
                        value: 'village',
                        label: 'Village nearby',
                        desc: '5 min away, we help coordinate',
                        availability: `${villageBedsRemaining} of ${totalVillageBeds} beds available`,
                      },
                      {
                        value: 'camping',
                        label: 'Camping',
                ...
                {/* Transport */}
                <div className="border-t border-white/20 pt-6">
                  <label className="block text-white font-semibold mb-3">How will you get there?</label>
                  ...
                  {formData.transportMode === 'train' && (
                    <label className="flex items-center gap-3 cursor-pointer mt-3 bg-white/10 rounded-xl p-3">
                      ...
                      />
                      <span className="text-white text-sm">I&apos;d need a shuttle from the train station</span>
                    </label>
                  )}
                </div>
              </div>
            ...
            {step === 5 && (
              <div className="flex-1">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-white font-semibold mb-2">Dietary restrictions (optional)</label>
                    <input
                      type="text"
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                      ...
                      <div>
                        ...
                        <span className="text-white/70 text-sm">
                          Bar shift, helping in the kitchen, setting up — whatever is needed.
                        </span>
                      </div>
                    </label>
                    ...
                    {formData.willingToHelp && (
                      <div className="mt-3">
                        <input
                          ...
                          className="w-full px-4 py-3 rounded-lg bg-white/90 text-[#2D4A3E] font-medium focus:outline-none focus:ring-2 focus:ring-[#FFE135]"
                          placeholder="Your skills: DJ, cooking, decoration, bar..."
                        />
                      </div>
                    )}
                  </div>
